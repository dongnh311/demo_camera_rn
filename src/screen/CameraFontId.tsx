...
import { CONTENT_SPACING, MAX_ZOOM_FACTOR, SAFE_AREA_PADDING } from '../const/Constrants';
...
type Props = NativeStackScreenProps<Routes, 'CameraFontId'>;
export function CameraFontId({ navigation }: Props): React.ReactElement {
    ...
    const setIsPressingButton = useCallback(
      (_isPressingButton: boolean) => {
        ...
        console.log(`Is press button ${_isPressingButton}`);
      },
      [isPressingButton],
    );
    ...
    const checkPermission = useCallback(
      async () => {
        if (await verifyPermissions() === true) {
    ...
    const verifyPermissions = async () => {
      let perm = [ PERMISSIONS.IOS.PHOTO_LIBRARY, ];
      if (Platform.OS == 'android') {
        console.log('Android permission');
          let permAdnroid = [ PERMISSIONS.ANDROID.READ_EXTERNAL_STORAGE, PERMISSIONS.ANDROID.WRITE_EXTERNAL_STORAGE ];
          checkMultiple(permAdnroid).then((statuses) => {
            ...
            if (statuses[PERMISSIONS.ANDROID.READ_EXTERNAL_STORAGE] === RESULTS.GRANTED && statuses[PERMISSIONS.ANDROID.WRITE_EXTERNAL_STORAGE] === RESULTS.GRANTED) {
              ...
              const result = requestMultiple(permAdnroid).then((status) => {
                console.log('Read', status[PERMISSIONS.ANDROID.READ_EXTERNAL_STORAGE]);
                console.log('Write', status[PERMISSIONS.ANDROID.WRITE_EXTERNAL_STORAGE]);
                if (status[PERMISSIONS.ANDROID.READ_EXTERNAL_STORAGE] === RESULTS.GRANTED && status[PERMISSIONS.ANDROID.WRITE_EXTERNAL_STORAGE] === RESULTS.GRANTED) {
                  return true;
                } else {
                  return false;
                }
        ...
        if (result !== 'granted') {
            Alert.alert('Insufficient permissions!', 'You need to grant library access permissions to use this app.', [
                { text: 'Okay' }
            ]);
            return false;
        }
    ...
    return (
      <View style={styles.container}>
        ...
        <CaptureButton
          ...
          setIsPressingButton={checkPermission}
        />
